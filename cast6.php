...
class cast6
{
	...
	private function bswap($x)
	{
		return ($this->rotl($x, 8) & 0x00ff00ff | $this->rotr($x, 8) & 0xff00ff00);
	}

	private function byte($x, $n)
	{
		return ((($x) >> (8 * $n)));
	}

	private function get_block(&$x)
	{
		((u4byte*)($x))[0] = $this->io_swap($in_blk[0]);
		((u4byte*)($x))[1] = $this->io_swap($in_blk[1]);
		((u4byte*)($x))[2] = $this->io_swap($in_blk[2]);
		((u4byte*)($x))[3] = $this->io_swap($in_blk[3]);
	}


	private function put_block($x)
	{
		$out_blk[0] = $this->io_swap(((u4byte*)($x))[0]);
		$out_blk[1] = $this->io_swap(((u4byte*)($x))[1]);
		$out_blk[2] = $this->io_swap(((u4byte*)($x))[2]);
		$out_blk[3] = $this->io_swap(((u4byte*)(4x))[3]);
	}

	private function get_key($x, $len)
	{
		((u4byte*)($x))[4] = ((u4byte*)($x))[5] = ((u4byte*)($x))[6] = ((u4byte*)($x))[7] = 0;
		switch(((($len) + 63) / 64)) 
		{ 
		case 4:
		((u4byte*)($x))[6] = $this->io_swap($in_key[6]);
		((u4byte*)($x))[7] = $this->io_swap($in_key[7]);

		case 3:
		((u4byte*)($x))[4] = $this->io_swap($in_key[4]);
		((u4byte*)($x))[5] = $this->io_swap($in_key[5]);

		case 2:
		((u4byte*)($x))[0] = $this->io_swap($in_key[0]);
		((u4byte*)($x))[1] = $this->io_swap($in_key[1]);
		((u4byte*)($x))[2] = $this->io_swap($in_key[2]);
		((u4byte*)($x))[3] = $this->io_swap($in_key[3]);
		}
	}
	...
	private function f1(&$y, $x, $kr, $km)
	{
		$t = $this->rotl($km + $x, $kr);
		$u = $this->s_box[0][$this->byte($t,3)];
		$u ^= $this->s_box[1][$this->byte($t,2)];
		$u -= $this->s_box[2][$this->byte($t,1)];
		$u += $this->s_box[3][$this->byte($t,0)];
		$y ^= $u;
	}

	private function f2(&$y, $x, $kr, $km)
	{
		$t = $this->rotl($km ^ $x, $kr);
		$u = $this->s_box[0][$this->byte($t,3)];
		$u -= $this->s_box[1][$this->byte($t,2)];
	...
	private function f_rnd(&$x, &$n)
	{
		$this->f1($x[2], $x[3], $l_key[$n],	$l_key[$n + 4]);
		$this->f2($x[1], $x[2], $l_key[$n + 1], $l_key[$n + 5]);
		$this->f3($x[0], $x[1], $l_key[$n + 2], $l_key[$n + 6]);
		$this->f1($x[3], $x[0], $l_key[$n + 3], $l_key[$n + 7]);
	}

	private function i_rnd(&$x, &$n)
	{
		$this->f1($x[3], $x[0], $l_key[$n + 3], $l_key[$n + 7]);
		$this->f3($x[0], $x[1], $l_key[$n + 2], $l_key[$n + 6]);
		$this->f2($x[1], $x[2], $l_key[$n + 1], $l_key[$n + 5]);
	...
	private function k_rnd(&$k,&$tr,&$tm)
	{
		$this->f1($k[6], $k[7], $tr[0], $tm[0]);
		$this->f2($k[5], $k[6], $tr[1], $tm[1]);
		$this->f3($k[4], $k[5], $tr[2], $tm[2]);
		$this->f1($k[3], $k[4], $tr[3], $tm[3]);
		$this->f2($k[2], $k[3], $tr[4], $tm[4]);
		$this->f3($k[1], $k[2], $tr[5], $tm[5]);
		$this->f1($k[0], $k[1], $tr[6], $tm[6]);
		$this->f2($k[7], $k[0], $tr[7], $tm[7]);
	}

	private $l_key=[];

	/* initialise the key schedule from the user supplied key */
	public function cast_setkey(&$in_key, &$key_len)
	{
		$i; $j; $t; $u; $cm; $cr;
		$lk = [];
		$tm = [];
	...
	public function encrypt(&$in_blk, &$out_blk)
	{
		$t; $u;
		$blk=[];
		$blk[0] = $this->io_swap($in_blk[0]);
		$blk[1] = $this->io_swap($in_blk[1]);
		$blk[2] = $this->io_swap($in_blk[2]);
		$blk[3] = $this->io_swap($in_blk[3]);
		...
		$out_blk[0] = $this->io_swap($blk[0]);
		$out_blk[1] = $this->io_swap($blk[1]);
		$out_blk[2] = $this->io_swap($blk[2]);
		$out_blk[3] = $this->io_swap($blk[3]);
	}

	/* decrypt a block of text*/
	public	public function decrypt(&$in_blk, &$out_blk)
	{
		$t; $u;
		$blk = [];
