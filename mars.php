...
class mars
{
	...
	private function f_mix(&$a,&$b,&$c,&$d)
	{
			$r = $this->rotr($a, 8);
			$b ^= $this->s_box[$a & 255];
			$b += $this->s_box[($r & 255) + 256];
			$r = $this->rotr($a, 16);
			$a = $this->rotr($a, 24);
			$c += $this->s_box[$r & 255];
			$d ^= $this->s_box[($a & 255) + 256];
	}

	private function b_mix(&$a,&$b,&$c,&$d)
	{
		$r = $this->rotl($a, 8);
		$b ^= $this->s_box[($a & 255) + 256];
		$c -= $this->s_box[$r & 255];
		$r = $this->rotl($a, 16);
		$a = $this->rotl($a, 24);
		$d -= $this->s_box[($r & 255) + 256];
		$d ^= $this->s_box[$a & 255];
	}

	private function f_ktr(&$a,&$b,&$c,&$d, $i)
	{
		$m = $a + $this->l_key[$i];
		$a = $this->rotl($a, 13);
		$r = $a * $this->l_key[$i + 1];
	...
	private function r_ktr(&$a,&$b,&$c,&$d, $i)
	{
		$r = $a * $this->l_key[$i + 1];
		$a = $this->rotr($a, 13);
		$m = $a + $this->l_key[$i];
	...
	public function set_key(&$in_key, &$key_len)
	{
		...
		for($i = $j = 0; $i < 39; ++$i)
		{
		 	$this->vk[$i + 7] = $this->rotl($this->vk[$i] ^ $this->vk[$i + 5], 3) ^ $in_key[$j] ^ $i;
			$j = ($j === $m ? 0 : $j + 1);
		}
		$this->vk[46] = $key_len / 32;

		for($j = 0; $j < 7; ++$j)
		{
			 for($i = 1; $i < 40; ++$i)
			 {
				$this->vk[$i + 7] = $this->rotl($this->vk[$i + 7] + $this->s_box[$this->vk[$i + 6] & 511], 9);
			 }
			$this->vk[7] = $this->rotl($this->vk[7] + $this->s_box[$this->vk[46] & 511], 9);
		}
		...
		for($i = 5; $i < 37; $i += 2)
		{
			...
			if($m = $this->gen_mask($w))
			{
				$w ^= (rotl($this->s_box[265 + ($this->l_key[$i] & 3)], $this->l_key[$i + 3] & 31) & $m);
			}
			$this->l_key[$i] = $w;
		}
		return $this->l_key;
	...
	public function encrypt(&$in_blk, &$out_blk)
	{
		...
		$a = $in_blk[0] + $this->l_key[0];
		$b = $in_blk[1] + $this->l_key[1];
		$c = $in_blk[2] + $this->l_key[2];
		$d = $in_blk[3] + $this->l_key[3];
		...
		$out_blk[0] = $a - $this->l_key[36];
		$out_blk[1] = $b - $this->l_key[37];
		$out_blk[2] = $c - $this->l_key[38];
		$out_blk[3] = $d - $this->l_key[39];
	}

	public function decrypt(&$in_blk, &$out_blk)
	{
		$a; $b; $c; $d; $l; $m; $r;
		$d = $in_blk[0] + $this->l_key[36];
		$c = $in_blk[1] + $this->l_key[37];
		$b = $in_blk[2] + $this->l_key[38];
		$a = $in_blk[3] + $this->l_key[39];
		...
		$d -= $a;
		$this->b_mix($d, $a, $b, $c);
		$out_blk[0] = $d - $this->l_key[0];
		$out_blk[1] = $c - $this->l_key[1];
		$out_blk[2] = $b - $this->l_key[2];
		$out_blk[3] = $a - $this->l_key[3];
	}
}
?>